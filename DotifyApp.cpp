...
void DotifyApp::autogenerate() {
    ...
    cout << "What is the category you’d like to autogenerate a playlist with? " <<
        "(NAME/ARTIST/ALBUM/MAGIC)" << endl;
    category = getInput();
    if (category != "NAME" && category != "ARTIST" && category != "ALBUM" && category != "MAGIC") {
        cout << "\"" << category << "\" is not a valid criterion to autogenerate a playlist with." << endl;
        return;
    }
...
// Autogenerate a playlist by name
void DotifyApp::agPlaylist(string category, string query) {
    if (dotDriver.containsPlaylistWithName(query)) {
        cout << "\"" << query << "\" playlist already exists." << endl;
        return;
    }
    
    // Get a vector of songs containing the query
    vector<shared_ptr<LibrarySong>> agLibrary = dotDriver.agLibrary(query, category);
    
    // Create a new playlist with the name of the query
    shared_ptr<Playlist> newPlaylist(new Playlist(query));
    dotDriver.addPlaylist(newPlaylist);
    
    // Adds songs from the above vector into the new playlist
    for (int i = 0; i < agLibrary.size(); i++) {
        dotDriver.addSongToPlayList(query, agLibrary.at(i));
    }
    cout << "\"" << query << "\" playlist autogenerated successfully." << endl;
}

// Autogenerate a playlist by magic
void DotifyApp::agMagic() {
